...
| Output Length (Leva 3) | `max_tokens` calibrati su output realistico per ogni agente: DC 4096→1024, NS 4096→2048, FA 4096→2048, RA 4096→2048, RW 16000→8000, PM 4096→2048. Invariati: RW QA 2048, LLMJudge 1024, HarnessAnalyzer 2048, TaskDecomposer 10000 (deve eccedere thinking budget 8000). Previene output verbosi; segnala al modello la dimensione attesa dell'output. | Reverificare DC (1024) e RW (8000) con token reali in Fase 5 |
